    ...
    (let ((action (car args))
      ...
      (if (not (or (= (length conditions) 0) (= (remainder (length conditions) 3) 0)))
          ...
          (let* ((shapes (read-shapes filename))
            ...
            (cond
             ((string=? action "count") (begin (display "There are ") (display (length filtered)) (display " shapes.") (newline)))
             ((string=? action "print") (begin (for-each format-shape filtered) (display "")))
             ((string=? action "min") (let ((m (apply min (map volume filtered))) ) (display m) (newline) m))
             ((string=? action "max") (let ((m (apply max (map volume filtered))) ) (display m) (newline) m))
             ((string=? action "total") (let ((t (apply + (map volume filtered))) ) (display t) (newline) t))
             ((string=? action "avg") (let ((sum (apply + (map volume filtered)))
                                             (cnt (length filtered)))
                                         (if (> cnt 0)
